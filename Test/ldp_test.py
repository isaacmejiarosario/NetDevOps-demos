    ...
    def mpls_ldp_status(self):
        for device in self.parameters["testbed"].devices.values():
            ...
            if os == "iosxr":
                ...
                with open(f"./Test/golden_ops/{device.name}_show-mpls-ldp-neighbor_parsed.txt", "rb") as f:
                ...
                with open(f"./Test/golden_ops/{device.name}_show-ldp-session_parsed.txt", "rb") as f:
                    golden_template = json.load(f)
