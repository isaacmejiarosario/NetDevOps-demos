    ...
    def ospf_status(self):
        for device in self.parameters["testbed"].devices.values():
            ...
            if os == "iosxr":
                ...
                with open(f"./Test/golden_ops/{device.name}_show-ospf-vrf-all-inclusive-neighbor-detail_parsed.txt", "rb") as file:
                ...
                with open(f"./Test/golden_ops/{device.name}_show-ospf-neighbor-instance-all_parsed.txt", "rb") as file:
                    golden_template = json.load(file)
